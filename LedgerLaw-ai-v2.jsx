...
import { generateDemand } from "./src/lib/api";
...
// ════════════════ NEW DEMAND ════════════════
const NewDemand=()=>{
const [step,setStep]=useState(1);const [gen,setGen]=useState(false);const [done,setDone]=useState(false);const [err,setErr]=useState(null);
const [caseData,setCaseData]=useState({clientName:"Maria Gonzalez",caseType:"Motor Vehicle Accident",accidentDate:"2025-10-15",carrier:"Allstate",policyLimit:"$100,000",attorney:"Emery Brett Ledger",accidentSummary:"Rear-ended at red light on I-5 near Mission Viejo. At-fault driver texting."});
const [result,setResult]=useState(null);
const go=async()=>{setGen(true);setErr(null);try{const r=await generateDemand(caseData);setResult(r);setDone(true);setStep(4);}catch(e){setErr(e.message);}finally{setGen(false);}};
...
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}><div><label>Client Name</label><input value={caseData.clientName} onChange={e=>setCaseData(d=>({...d,clientName:e.target.value}))}/></div><div><label>Case Type</label><select value={caseData.caseType} onChange={e=>setCaseData(d=>({...d,caseType:e.target.value}))}>{CTYPES.map(c=><option key={c}>{c}</option>)}</select></div><div><label>Accident Date</label><input type="date" value={caseData.accidentDate} onChange={e=>setCaseData(d=>({...d,accidentDate:e.target.value}))}/></div><div><label>Insurance Carrier</label><select value={caseData.carrier} onChange={e=>setCaseData(d=>({...d,carrier:e.target.value}))}>{CARRIERS.map(c=><option key={c}>{c}</option>)}</select></div><div><label>Policy Limit</label><input value={caseData.policyLimit} onChange={e=>setCaseData(d=>({...d,policyLimit:e.target.value}))}/></div><div><label>Attorney</label><input value={caseData.attorney} onChange={e=>setCaseData(d=>({...d,attorney:e.target.value}))}/></div></div>
<div style={{marginBottom:12}}><label>Accident Summary</label><textarea value={caseData.accidentSummary} onChange={e=>setCaseData(d=>({...d,accidentSummary:e.target.value}))}/></div>
...
{err&&<div style={{padding:10,marginBottom:10,borderRadius:8,background:T.redDim,border:`1px solid ${T.red}33`,color:T.red,fontSize:12}}>{err}</div>}
<div style={{display:"flex",justifyContent:"space-between"}}><Btn v="secondary" onClick={()=>setStep(2)}>← Back</Btn><Btn icon={gen?<Spin/>:I.zap} onClick={go} disabled={gen}>{gen?"Generating...":"Generate Demand"}</Btn></div></Card>}
{step===4&&done&&result&&<Card style={{background:`linear-gradient(135deg,${T.greenDim},${T.steelDim})`}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><div style={{width:34,height:34,borderRadius:"50%",background:T.green,color:T.white,display:"flex",alignItems:"center",justifyContent:"center"}}>{I.chk}</div><div><div style={{fontSize:15,fontWeight:700,color:T.green}}>Demand Generated</div><div style={{fontSize:11.5,color:T.txM}}>{(result.pages||1)} pages · Colossus-optimized · Claude Sonnet 4</div></div></div>
<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>{[{v:String(result.pages||1),l:"Pages"},{v:"—",l:"ICD Codes"},{v:"—",l:"Exhibits"},{v:"—",l:"SSN Redacted"}].map(x=><div key={x.l} style={{padding:8,borderRadius:8,background:T.srf,textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:T.steel}}>{x.v}</div><div style={{fontSize:10,color:T.txM}}>{x.l}</div></div>)}</div>
<div style={{maxHeight:200,overflow:"auto",padding:12,marginBottom:14,background:T.srf,borderRadius:8,border:`1px solid ${T.bdr}`,fontSize:11.5,color:T.tx2,whiteSpace:"pre-wrap"}}>{result.demand}</div>
